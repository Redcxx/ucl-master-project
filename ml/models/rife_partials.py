...
def warp(tenInput, tenFlow, opt):
    ...
    return F.grid_sample(input=tenInput, grid=g, mode='bilinear', padding_mode='border',
                         align_corners=True)
...
class ContextNet(nn.Module):
    ...
    def forward(self, x, flow):
        ...
        f1 = warp(x, flow, self.opt)
        x = self.conv2(x)
        flow = F.interpolate(flow, scale_factor=0.5, mode="bilinear", align_corners=False,
                             recompute_scale_factor=False) * 0.5
        f2 = warp(x, flow, self.opt)
        x = self.conv3(x)
        flow = F.interpolate(flow, scale_factor=0.5, mode="bilinear", align_corners=False,
                             recompute_scale_factor=False) * 0.5
        f3 = warp(x, flow, self.opt)
        x = self.conv4(x)
        flow = F.interpolate(flow, scale_factor=0.5, mode="bilinear", align_corners=False,
                             recompute_scale_factor=False) * 0.5
        f4 = warp(x, flow, self.opt)
        return [f1, f2, f3, f4]
...
class Unet(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Conv2d(c, 3, (3, 3), (1, 1), 1)
...
class IFBlock(nn.Module):
    def __init__(self, in_planes, c=64):
        ...
        self.lastconv = nn.ConvTranspose2d(c, 5, (4, 4), (2, 2), (1, 1))
...
class IFNet_m(nn.Module):
    def __init__(self, opt):
        ...
        self.contextnet = ContextNet(self.opt)
        self.unet = Unet()
    ...
    def forward(self, x, scale=[4, 2, 1], timestep=0.5):
        ...
        for i in range(3):
            ...
            warped_img0 = warp(img0, flow[:, :2], self.opt)
            warped_img1 = warp(img1, flow[:, 2:4], self.opt)
        ...
        if gt.shape[1] == 3:
            ...
            warped_img0_teacher = warp(img0, flow_teacher[:, :2], self.opt)
            warped_img1_teacher = warp(img1, flow_teacher[:, 2:4], self.opt)
